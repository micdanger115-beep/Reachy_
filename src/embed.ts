...
import type { StateEventDetail } from "@pollen-robotics/reachy-mini-sdk";
...
type Handle = Awaited<ReturnType<typeof connectToHost>>;

async function main() {
  const handle: Handle = await connectToHost();
  ...
  // ---------------------------------------------------------------- Video/Audio
  const video = $<HTMLVideoElement>("#robot-video");
  video.autoplay = true;
  ...
  const hearVol = $<HTMLInputElement>("#hear-volume");
  ...
  const hearMuteBtn = $<HTMLButtonElement>("#hear-mute");
  let hearMuted = false;
  ...
  const lockBtn = $<HTMLButtonElement>("#talk-lock");
  const micStatus = $("#mic-status");

  const getAudioSender = (): RTCRtpSender | null => {
    const pc = reachy._pc;
    if (!pc) return null;
  ...
  const enableMic = async () => {
    try {
      ...
      if (!micTrack) throw new Error("Kein Mikrofon-Track");
      micTrack.enabled = false;
      const sender = getAudioSender();
      if (!sender) throw new Error("Kein Audio-Sender in der WebRTC-Verbindung gefunden");
  ...
  $<HTMLButtonElement>("#mic-enable").addEventListener("click", enableMic);

  // Push-to-Talk (halten zum Sprechen)
  const pttDown = (e: Event) => {
    if (micLocked || !micTrack) return;
    e.preventDefault();
    setTalking(true);
  };
  const pttUp = () => {
  ...
  // -------------------------------------------------------- Reachy-Lautstärke
  const speakVol = $<HTMLInputElement>("#speak-volume");
  ...
  // ------------------------------------------------------------- Body-Yaw
  const bodySlider = $<HTMLInputElement>("#body-yaw");
  ...
  // ------------------------------------------------------------- Antennen
  const antSlider = $<HTMLInputElement>("#antennas");
  ...
  $<HTMLButtonElement>("#center").addEventListener("click", () => {
    curPitch = 0;
    curYaw = 0;
    knob.style.left = "50%";
    knob.style.top = "50%";
...
// ================================================================== UI-Template
function template(userName: string | null, appName: string): string {
  return `
  <div class="app">
